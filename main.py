...
selected_features = ['Restaurant_latitude', 'Restaurant_longitude', 'Delivery_location_latitude',
                     'Delivery_location_longitude', 'Distance', 'Delivery_person_Age', 'Type_of_vehicle']
...
def application():
    ...
    streamlit_write(header='h5',
                    text="Σκοπός του προγράμματος είναι ο υπολογισμός της εκτιμώμενης παράδοσης μιας παραγγελίας με μεγάλο ποσοστό ευστοχίας.")
    st.markdown('---')
    # ---- HEADER: Μικρή επεξήγηση για την λειτουργία του προγράμματος  ----

    # ---- SECTION: Food Delivery Time Calculator ----
    training_algorithm = st.selectbox("Training algorithm 🤖",
                                      ["Random Forest Regression", "Gradient Boosting Regression", "Linear Regression",
                                       "Decision Tree Regression", "Extra Trees Forest", "K-Neighbors Regression"])
    ...
    with col2:
        ...
        restaurant_location_longitude = st.number_input("Restaurant location longtitude🗺️", min_value=-180,
                                                        max_value=180)
...
def data_analysis():
    ...
    streamlit_write(header='h5',
                    text="Στην εξερευνητική ανάλυση δεδομένων αυτή, θα γίνει επεξεργασία, επισκόπηση καθώς και καθαρισμός των δεδομένων για την βελτίωση της ποιότητάς τους και την αποτελεσματικότητα της ανάλυσης.")
    # ---- HEADER: Μικρή εισαγωγή για την σελίδα της εξερευνητικής ανάλυσης δεδομένων ----
    ...
    streamlit_write(header='h5',
                    text="Εμφάνιση των πρώτων 5 γραμμών του dataframe, για να βεβαιωθούμε ότι εισαγωγή των δεδομένων εκτελέστηκε επιτυχώς.")
    ...
    streamlit_write(header='h5',
                    text="Όπως βλέπουμε σχεδόν 46.000 εγγραφές, συγκριτικά μικρό με άλλα datasets αλλά επαρκές για την περίπτωση μας.")
    # ---- SECTION 1: Εμφάνιση του dataframe ----
    ...
    streamlit_write(header='h5',
                    text=" Ευτυχώς, τα δεδομένα μας είναι ολοκληρομένα, αλλά συνήθως ο καθαρισμός είναι απαραίτητο κομμάτι της Διαχείρηση Μεγάλου Όγκου Δεδομένων.")
    ...
    streamlit_write(header='h5',
                    text=" Επιπλέον έλεγχος για τον καθαρισμό δεδομένων, όπου ελέγχουμε αν υπάρχουν διπλότυπες στύλες μέσα στο dataset μας")
    ...
    streamlit_write(header='h5',
                    text="Για άλλη μια φορά, δεν ύπαρχουν διπλότυπα στα δεδομένα μας, το οποίο σημαίνει ότι είναι ολοκληρομένα.\n")
    ...
    streamlit_write(header='h5',
                    text="Από ότι βλέπουμε, όχι μόνο δεν έχουμε πρόβλημα με τα δεδομένα μας, αλλά έχουμε μια ίση σχεδόν κατανομή των πεδίων. Το οποίο μας γλυτώνει την δουλειά της κανονικοποίησης. Ας συνεχίσουμε στο επόμενο βήμα.")

    # Εφαρμογή του Five Number Summary στα δεδομένα μας
    streamlit_write(header='h5',
                    text="Ας κάνουμε το Five Number Summary (minimum, first quartile, median, third quartile, and maximum) και να δούμε τι θας μας βγάλει:")
    ...
    streamlit_write(header='h5',
                    text="Σε αυτό το κομμάτι θα κάνουμε χρήση βιβλιοθηκών όπως `matplotlib` και `seaborn` για την οπτικοποίηση των δεδομένων μας.")
    ...
    st.pyplot(plt)
    streamlit_write(header='h5',
                    text="Όπως παρατηρήσαμε και στο αριθμητικό παράδειγμα, η κατανομή των πεδίων μας είναι ίση, το οποίο μας γλυτώνει την κανονικοποίηση των δεδομένων.")
    ...
    streamlit_write(header='h5',
                    text="Ας κάνουμε το ίδιο για των τύπων οχημάτων που χρησιμοποιήθηκαν ('Type_of_vehicle'):")
    st.pyplot(plt)
    ...
    streamlit_write(header='h5',
                    text="Και στην συνέχεια, ας κάνουμε την οπτική αναπάρασταση των 5 καλύτερων διανομέων, για να δούμε το γενικό count, και την απόδοση τους.")
    st.pyplot(plt)
    ...
    streamlit_write(header='h5',
                    text="Κάνοντας το heatmap μπορούμε να διαπυστώσουμε σε ποια πεδία τιμών να εστιάσουμε, και γενικότερα έχουν την μεγαλύτερη συσχέτιση μεταξύ τους.")
    st.pyplot(plt)
    streamlit_write(header='h5',
                    text="Όπως βλέπουμε, τα πεδία που έχουν την σημαντικότερη συσχέτιση είναι αυτά που έχουν να κάνουνε με την τοποθεσία του εστιατορίου/μαγαζιού (`'Restaurant_latitude', 'Restaurant_longitude'`) και αντίστοιχα του πελάτη ή της παραγγελίας (`'Delivery_location_latitude', 'Delivery_location_longitude'`).")
    streamlit_write(header='h5',
                    text="Άρα, στην ερεύνηση και ανάπτυξη της εφαρμογής μας για τον υπολογισμό της αναμενόμενης ώρας για μια παραγγελία, θα πάρουμε αυτά τα πεδία υπόψη μας, καθώς έχουν την μεγαλύτερη βαρύτητα στην πρόβλεψη του χρόνου παράδοσης.")

    # Αναπαράσταση της σχέσης χρόνου κάθε στήλης
    time_taken_corr_dataframe = data_frame[numeral_columns].corr()['Time_taken(min)'].reset_index().sort_values(
        'Time_taken(min)')[:-1]
    plt.figure(dpi=100)
    ...
    plt.xticks(rotation=90);
    streamlit_write(header='h5',
                    text="Συνεπώς, ας βρούμε την συσχέτιση του χρόνου (time taken), με κάθε πεδίου της στήλης μας.")
    st.pyplot(plt)
    streamlit_write(header='h5',
                    text="Διαγραμματικά μας δείχνετε, ότι όσον αφορά την σχέση χρόνου του πεδίου αξιολόγησης (`'Delivery_person_ratings'`), έχουμε αρνητική συσχέτιση. Δηλαδή, η επιρροή του στο χρόνο είναι ανύπαρκτη.")
    streamlit_write(header='h5',
                    text="Αντιθέτως, η σχέση χρόνο με το πεδίο της ηλικίας του διανομέα (`'Delivery_person_age'`), φαίνεται να έχει την μεγαλύτερη συσχέτιση. Και ως επακόλουθο, κατά τον υπολογισμό και πρόβλεψη του χρόνου διανομής, θα πάρουμε υπόψη την ηλικία του διανομέα.")
    # ---- SECTION 3: Οπτικοποίηση δεδομένων ----
...
def read_me():
    ...
    streamlit_write(header='h5',
                    text="Σκοπός του παρόν αρχείου είναι να δείξουμε συνοπτικά τις βιβλιοθήκες που χρησιμοποιήσαμε και τον τρόπο εκτέλεσης της εφαρμογής.")
    st.markdown('---')
    # ---- HEADER: Μικρή επεξήγηση για τις βιβλιοθήκες που χρησιμοποιήσαμε και τον τρόπο εκτέλεσης της εφαρμογής  ----
